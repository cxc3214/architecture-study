...
public class ClassUtils {
	private static final String CLASS_PATH = ClassUtils.class.getClassLoader()
			.getResource("").getPath();
	...
	public static List<Class> scanPackage(String packageName, ClassFilter classFilter) {
		...
		scanFiles(classFileDirectory, classList,
				classFilter == null ? DEFAULT_CLASS_FILTER : classFilter);
		return classList;
	}
	...
	private static Class getClass(File file, ClassFilter classFilter)
			throws ClassNotFoundException {
		final String packagePath = file.getAbsolutePath().substring(
				file.getAbsolutePath().indexOf(CLASS_PATH)
						+ CLASS_PATH.length());
	...
	private static String obtainFilePath(String packageName) {
		final String oppositeFilePath = packageName
				.replace(DOT, File.separator);
		return CLASS_PATH + oppositeFilePath;
	}
	
//	public static void main(String[] args) {
//		List<Class> tlist = ClassUtils.scanPackage("net.imspring.study.mvc",new ControllerFilter());
//		for(Class cs :tlist){
